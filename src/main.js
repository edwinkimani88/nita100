...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // -------------------------------------------------------------
  // 7. JOURNEY TO 100: PINNED HORIZONTAL TIMELINE
  // -------------------------------------------------------------
  const isMobileSize = () => window.innerWidth < 1024;

  if (gsapInstance && ScrollTriggerInstance && !isMobileSize()) {
    ...
    if (timelineContainer && scrollWrapper) {
      // Calculate total translation width dynamically
      const wrapperWidth = scrollWrapper.scrollWidth;
      const windowWidth = window.innerWidth;
      const amountToScroll = wrapperWidth - windowWidth + 120; // safe padding

      // Pin the container and translate the timelines
      gsapInstance.to(scrollWrapper, {
        ...
        scrollTrigger: {
          ...
          scrub: 1.0,
          start: "top top",
          end: `+=${amountToScroll * 1.5}`, // Elegant speed
          invalidateOnRefresh: true,
          onUpdate: (self) => {
            // Animate glowing connection line following progress
            const lineProgress = self.progress * 100;
            const glowLine = document.getElementById("timeline-glow-line");
            if (glowLine) glowLine.style.width = `${lineProgress}%`;
          }
        }
      });
    }
  ...
  if (gsapInstance && ScrollTriggerInstance) {
    ...
    fadeSections.forEach((section) => {
      ...
      gsapInstance.fromTo(section.querySelectorAll(".fade-up-item"),
        { opacity: 0, y: 50 },
        {
          opacity: 1,
          y: 0,
